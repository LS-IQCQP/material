...
namespace solver
{
    ...
    void qp_solver::judge_problem()
    {
        ...
        if (bool_size > 0)
        {
            ...
            else if (int_size != 0) problem_type = 1;
            else problem_type = 2;
        }
        else 
        {
    ...
    void qp_solver::local_search()
    {
        ...
        if (_int_vars.size() > _bool_vars.size()) local_search_with_real();
    ...
    void qp_solver::no_operation_walk_unsat()
    {
        ...
        {
            if (_int_vars.find(var_idx)!=_int_vars.end()) change_value = (floor(_vars[var_idx].lower));
    ...
    void qp_solver::initialize()
    {
        ...
        //         score += cur_score;
        //         bin_var->constraints_score[con_size] = cur_score;
        //     }
    ...
    {
        if (_vars[var_idx].equal_bound) return false;
        if (fabs(change_value) < 1e-7) return false;
    ...
    void qp_solver::print_best_solution()
    {
        ...
        {
            // cout << "bound" << index << ": ";
            cout <<_vars[index].name << "     "<< sol << endl;
            index++;
        }
    }
}