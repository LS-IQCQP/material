...
namespace solver
{
    ...
    void qp_solver::insert_var_change_value_comp(int var_pos, Float change_value_1, int var_idx, all_coeff * a_coeff, Float delta, polynomial_constraint * pcon, bool rand_flag)
    {
        ...
        if (a_coeff->obj_quadratic_coeff != 0) return;
        if (true)
        {
    ...
    bool qp_solver::compensate_move()
    {
        ...
        //portfolio一下这里，再想想有没有别的乱七八糟的情况晖影响速度
        if (false)
        {
        ...
        for (int i = 0; i < _operation_vars.size(); i++)
        {
            ...
            for (int con_size : *nor_var->constraints)
            {
                ...
                pcon = & (_constraints[con_size]);
                if (!pcon->is_sat || pcon->monomials->size() > 50) continue;
                mono_delta = 0;
                for (int mono_pos = 0; mono_pos < pcon->monomials->size(); mono_pos++)
                {
                    mono_delta += pro_mono(pcon->monomials->at(mono_pos));
            ...
            // _cur_assignment[var_pos] -= change_value;
            _cur_assignment[var_pos] = cur_value;
            if (_operation_vars_pair.size() > 1000) break;
            if (TimeElapsed() > _cut_off || terminate) break;
        }
    ...
    void qp_solver::insert_operation_balance()
    {
        ...
        if (attenuation_flag)
        {
            for (int i = 0; i < _operation_vars.size(); i++) attenuation(_operation_vars[i], _operation_value[i]);
        }
        // for (int i = 0; i < _operation_vars.size(); i++)
        // {
    ...
    bool qp_solver::insert_var_change_value_balance(int var_idx, all_coeff * a_coeff, Float delta_unused, polynomial_constraint * pcon, Float symvalue, bool rand_flag)
    {
        ...
        all_coeff * first_coeff = &(pcon->var_coeff->at(var_idx));
        bool is_have = false;
    ...
    void qp_solver::local_search_mix_balance()
    {
        ...
        if (init_assignment) adjust_assignments();
        // int var_pos;
        int bin_var_pos;
        ...
        // sta_cons();
        if (portfolio_mode == 5) bms = 50;
        if (portfolio_mode == 7) bms = 200;
        for (_steps = 0; _steps <= _max_steps; _steps++)
        {
            //TODO
            if ((TimeElapsed() > _cut_off) || terminate) break;
            // if (_non_improve_step > 2000000) restart();
            if (is_cur_feasible)
            {
                ...
                if (max_score > 0)
                {
                    ...
                    // if (max_score == score)
                        ...
                        //     changes.emplace_back(var_pos, _cur_assignment[var_pos]);
            ...
            {
                if (portfolio_mode == 2 || portfolio_mode == -9)
                    propagate_move_unsat_mix(1, 5, 1, 1, true, false, true, true);
                else 
                {
            ...
            if (ls_assignment) adjust_assignments();
        }
        if (output_mode == 0)
        {
