...
namespace solver
{
    ...
    void qp_solver::execute_critical_move_no_cons(int var_pos, Float change_value)
    {
        ...
        if (change_value < 0) _vars[var_pos].last_pos_step = _steps + 3 + rds() % 10;
        else _vars[var_pos].last_neg_step = _steps + 3 + rds() % 10;
