...
namespace solver
{
                ...
                if (!with_quadratic)
                {
                    ...
                    if (coeff != 0) 
                    {
                        ...
                        else 
                        {
                            ...
                            cur_con->var_coeff->emplace(size, new_coeff);
                        }
                    }
                }
                else 
                {
                    ...
                    else 
                    {
                        ...
                        else 
                        {
                            ...
                            cur_con->var_coeff->emplace(size_2, new_coeff);
                        }
                    }
                }
                stage = 0;
    ...
    void qp_solver::read_bounds(string line)
    {
        ...
        if (read_vec.size() == 5)
        {
            ...
            if (read_vec[4] == "inf" || read_vec[4] == "infinity")
            {
    ...
    void qp_solver::print_imp_info()
    {
        ...
        for (auto var_idx : _vars_in_obj)
        {
            ...
            for (int linear_pos = 0; linear_pos < v->obj_linear_coeff->size(); linear_pos++)
            {
                ...
                cout<< v->obj_linear_constant_coeff->at(linear_pos) <<" * "<< _vars[v->obj_linear_coeff->at(linear_pos)].name;
